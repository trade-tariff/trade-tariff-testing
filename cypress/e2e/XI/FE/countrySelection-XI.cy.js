...
describe('🇪🇺 💡 | countrySelection-XI | Country Selection |', {tags: ['config', 'xbrowser-tag']}, function() {
  ...
  it('XI Country selection page', function() {
    ...
    cy.get('div:nth-of-type(6) > .govuk-summary-list__value').contains('Argentina');

    cy.get('.autocomplete__wrapper').contains('Argentina (AR)');
    // Typing the country code
    cy.get('div:nth-of-type(6) > .govuk-summary-list__actions > .govuk-link').click();
    cy.countryPickerpage({value: '(DE)'});
    cy.get('div:nth-of-type(6) > .govuk-summary-list__value').contains('Germany');

    // reset to all countries
    cy.get('.reset-country-picker').click();
    cy.get('div:nth-of-type(6) > .govuk-summary-list__value').contains('All countries');
  });
