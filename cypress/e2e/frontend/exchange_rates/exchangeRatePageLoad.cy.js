describe('validate /exchange_rates', function() {
  ...
  it('returns the latest monthly exchange rates', function() {
    ...
    cy.get('a[href="/api/v2/exchange_rates/files/monthly_csv_2023-9.csv"]').contains('CSV');

    // and can download CSV
    cy.request('/api/v2/exchange_rates/files/monthly_csv_2023-9.csv').then((response) => {
  ...
  it('displays exchange rates for previous years', function() {
    ...
    cy.get('a[href="/api/v2/exchange_rates/files/monthly_csv_2022-12.csv"]').contains('CSV');

    cy.request('/api/v2/exchange_rates/files/monthly_csv_2022-12.csv').then((response) => {
      assert.equal(response.status, 200);
    });
  });
});
