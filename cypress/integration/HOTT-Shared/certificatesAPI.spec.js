  ...
  it('XI certificates API - as of validate data | 1971 - 1 entry  ', function() {
    ...
    cy.get('@comments')
        .then((response) => {
          ...
          expect(response.body.data[0].attributes).to.have.property('validity_start_date');
          // CDS /CHIEF guidance link
          expect(response.body.data[1].attributes.guidance_cds).to.have.string('No information currently available');
          expect(response.body.data[1].attributes.guidance_chief).to.have.string('No information currently available');
        });
  });
