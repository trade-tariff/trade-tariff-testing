describe('Old UK 🇬🇧 💡 Main Page - headers ,sections  - (UK version)',function() {
    //--- Headings to be changed for UK -------------
    Cypress.config('baseUrl')

    //Page Title
    it('Old UK - Header text - sections page', function () {
        cy.visit('/sections')
        cy.get('.govuk-header').should('be.visible', 'The Online Trade Tariff')
    })
    //Gov Logo
    it('Old UK - GOV.UK logo ',function (){
        cy.visit('/sections')
        cy.get('.govuk-header').should('be.visible', 'GOV.UK')
    })

    it('OLD UK - Check correct date is displayed',function(){
        cy.visit('/sections')
        cy.get('.govuk-grid-column-full')
            .contains(Cypress.moment().format('DD MMMM YYYY'))
    })
    //Sub sections in headings
    it('Old UK - Header text - Sub sections on headings banner ', function () {
        cy.visit('/sections')
        cy.get('.govuk-header ')
    ...
    it('Old UK - Header text - Page - enter commodity code for Mozzaarella - 0406103010 and search',function(){
        cy.get('.js-commodity-picker-select.js-show  input#q').click().type('0406103010')
        cy.wait(1000)
        cy.get('input[name=\'new_search\']').click()
        cy.get('.govuk-header').should('be.visible', 'The Online Trade Tariff')
    })
    it('Old UK - Header text - Page - Commodity information for 0406103010 is displayed',function(){
        cy.get('.commodity-header.govuk-heading-l').should('be.visible')
        cy.get('.govuk-header').should('be.visible', 'The Online Trade Tariff')
    })

    it('Old UK - Header text - Page - chapter notes is visible',function(){
        cy.get('div:nth-of-type(1) > .govuk-heading-s').should('be.visible')
        cy.get('.govuk-header').should('be.visible', 'The Online Trade Tariff')
    })
    it('Old UK - Header text - page - select Export button',()=> {
        cy.get('a#tab_export').click()
        cy.get('div#import > .govuk-heading-m').contains('Import measures and restrictions')
        cy.get('.govuk-header').should('be.visible', 'The Online Trade Tariff')
    })
    it('Old UK - Header text - page - select Import button',()=> {
        cy.get('a#tab_import').click()
        cy.get('div#import > .govuk-heading-m').contains('Import measures and restrictions')
        cy.get('.govuk-header').should('be.visible', 'The Online Trade Tariff')
    })

    it('Old UK - Header text - page - select Chile from All countries list',()=>{
    ...
    //HOTT-164
    it('Old UK - Remove the link to the EU website for looking up measures, geographical areas and regulations - Main Page ',function(){
        cy.visit('/sections')
        cy.get('.govuk-footer')
    ...
    it('Old UK - Remove the link to the EU website for looking up measures, geographical areas and regulations - Terms and Conditions -page link ',function() {
        cy.visit('/sections')
        cy.get('.govuk-footer__inline-list > li:nth-of-type(3) > .govuk-footer__link').click()
        cy.contains('Summary')
        cy.contains('Integrated tariff of the European Community (TARIC) database').should('not.exist')
    })
    it('Old UK - Remove the link to the EU website for looking up measures, geographical areas and regulations - Terms and Conditions -URL',function(){
        cy.visit('/terms')
        cy.contains('Summary')
