...
context('🇪🇺 | ⚙️ XI - add declarable on V1 and V2 and set true/false for STW ', () => {
    ...
    it(' Validate API response for V2 | headings which are also commodities |', () => {
        cy.request({
            method: 'GET',
            url: '/xi/api/v2/commodities/9206000000'
        }).then((response) => {
            expect(response.status).to.eq(200);
            //console.log(JSON.stringify(response.body))
            expect(response.body).not.to.be.null
    ...
    it(' Validate API response for V1 | headings which are also commodities |', () => {
        cy.request({
            method: 'GET',
            url: '/xi/api/v1/commodities/9206000000'
        }).then((response) => {
            expect(response.status).to.eq(200);
            //console.log(JSON.stringify(response.body))
            expect(response.body).not.to.be.null
