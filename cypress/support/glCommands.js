...
const urlContains = (commodityCode) => {
...
Cypress.Commands.add('verifySpimmPage', () => {
    cy.contains('Check if you are eligible for the simplified processes for Internal Market Movements') +
...
Cypress.Commands.add('tellUsAboutYourGoodsPage', (commodityCode, originCountry, dateBooleanVal, originCountryName) => {
    const movingDate = (index) => `#green_lanes_moving_requirements_form_moving_date_${index}`;

    if (dateBooleanVal == true) {
        cy.url().should('include', `/check_simplified_processes_eligibility/your_goods/new?${urlContains(commodityCode)}`);
    } else {
    ...
    cy.get('#green-lanes-moving-requirements-form-country-of-origin-field').select(`${originCountryName}`);
...
Cypress.Commands.add('category1ExemptionsPage', (commodityCode, originCountry, documentCodes, dateBooleanVal) => {
    ...
    if (dateBooleanVal == true) {
        cy.url().should('include',
            `/category_exemptions/new?category=1&${urlContains(commodityCode)}`);
    } else {
        cy.url().should('include',
            `/check_simplified_processes_eligibility/category_exemptions?category=1&${urlContains(commodityCode)}`);
    }
...
// Tell us if your goods meet any exemptions - Category 2 Exemptions
Cypress.Commands.add('category2ExemptionsPage', (commodityCode, originCountry, documentCodes, withExemptions) => {
    ...
    if (withExemptions == true) {
        cy.get('@cat1').then((cat1ExemptionsUrlParams) => {
            cy.url().should('include',
                `/check_simplified_processes_eligibility/category_exemptions?${cat1ExemptionsUrlParams}c1ex=true&category=2&${urlContains(commodityCode)}`);
        });
    } else {
        cy.url().should('include',
            `/check_simplified_processes_eligibility/category_exemptions?category=2&${urlContains(commodityCode)}`);
    }
        ...
        if (onlyCat2Exemptions == true) {
            if (cat2DocCodes != 'none' || cat2DocCodes == 'none') {
                ...
                cy.get('@cat2').then((cat2ExemptionsUrlParams) => {
                    cy.url().should('include',
                        `/check_simplified_processes_eligibility/check_your_answers?${cat2ExemptionsUrlParams}c2ex=${c2exValue}&category=2&${urlContains(commodityCode)}`);
                });
            } else {
                cy.url().should('include',
                    `/check_simplified_processes_eligibility/check_your_answers?&c2ex=true&category=2&${urlContains(commodityCode)}`);
            }
            ...
            cy.get('@cat1').then((cat1ExemptionsUrlParams) => {
                cy.url().should('include',
                    `/check_simplified_processes_eligibility/check_your_answers?${cat1ExemptionsUrlParams}c1ex=${c1exValue}&category=1&${urlContains(commodityCode)}`);
            });
            ...
            cy.get('@categoriesData').then((categoriesDataDict) => {
                ...
                cy.url().should('include',
                    `/check_simplified_processes_eligibility/check_your_answers?${categoriesData}c1ex=${c1exValue}&c2ex=${c2exValue}&category=2&${urlContains(commodityCode)}`);
            });
        } else {
            cy.url().should('include',
                `/check_simplified_processes_eligibility/check_your_answers?${urlContains(commodityCode)}`);
...
Cypress.Commands.add('verifyResultPage', (commodityCode, originCountry, categoryResult, cat1DocCodes, cat2DocCodes,
    ...
    if (`${categoryResult}` == 'Category 1') {
        ...
        cy.contains('Category 1 Goods are not eligible to move using the simplified processes for Internal Market Movements');
...
Cypress.Commands.add('verifyPageURLsAndHeadingsByClickingBackLink', (data, docCodes, assertData, pageToVerify) => {
    if (pageToVerify == 'CYA') {
        ...
        // We need more information about your goods - cat 1 and select at least one exception from each Exemptions list
        cy.tellUsAboutYourGoodsPage(data[0], data[1], true, data[3]);
